...
parser.add_argument('--log_path', default='log2/', type=str,
                    help='log save path')
...
if dataset == 'sysu':
    ...
    data_path = '../autodl-tmp/Datasets/RegDB/'
    n_class = 206
    test_mode = [2, 1]
...
suffix = suffix + '_m_{}_focal_1.5'.format(args.mode)
test_log_file = open(args.log_path + suffix + '.txt', "w")
sys.stdout = Logger(args.log_path  + suffix + '_os.txt')
...
if dataset == 'sysu':
    ...
    print('==> Resuming from checkpoint..', file=test_log_file)
    ...
    for trial in range(10):
        ...
        if os.path.isfile(model_path):
            ...
            print('==> loading checkpoint {}'.format(model_path), file=test_log_file)
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
        ...
        print(
            'POOL:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
                cmc_pool[0], cmc_pool[4], cmc_pool[9], cmc_pool[19], mAP_pool, mINP_pool), file=test_log_file)
